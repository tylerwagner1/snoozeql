...
const ScheduleEditPage = () => {
  ...
  const [form, setForm] = useState({
    ...
    wake_cron: '0 7 * * *',
    enabled: true,
    selectors: [{ name: { pattern: '', type: 'exact' } }],
  })
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      try {
        const data: Schedule = await api.getSchedule(id)
        setForm({
          name: data.name || '',
          description: data.description || '',
          timezone: data.timezone || 'America/New_York',
          sleep_cron: data.sleep_cron || '0 22 * * *',
          wake_cron: data.wake_cron || '0 7 * * *',
          enabled: data.enabled ?? true,
          selectors: (data.selectors && data.selectors.length > 0) 
            ? data.selectors 
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {success && (
        <div className="mb-6 p-4 bg-green-500/10 border border-green-500/30 rounded-lg flex items-center gap-2 text-green-400">
          ...
          <span>Schedule updated successfully! Redirecting...</span>
        </div>
      )}
